...
template <typename T>
 class Audio<T,2>{
	...
	// unit test constructor
	Audio(int countbit, int sampfreq,  int sampnum,std::initializer_list<int> testlist) : samplenum(sampnum), bitnum(countbit), fs(sampfreq) {
	...
	void save(std::string file){
		...
		std::string name = file + "_" + std::to_string(fs) +"_"+std::to_string(bitnum) +"bit_stereo.raw";
		std::ofstream outputfile(name, std::ios::binary);
				//creating temporary array
	...
	Audio add(const Audio & other){
		...
		int startsmpl = cutrange.first*fs*2;
